...
Mixins have been added to work using [Jake Archibald's Sass-IE method](http://goo.gl/uwyT6), meaning grids for IE are *automatically* created with their media queries stripped out (so no need for an `ie.css` stylesheet). Include your grids per-breakpoint using `@include respond-min(30em) { @include grid-include("bp1-col")` to pass each class to be used in your HTML.